 ...
 * 
 */
public class AddAttributesPortfolioPage extends TestBase {
	...
	public static final Logger log = Logger.getLogger(AddAttributesPortfolioPage.class.getName());



	//Class constructor
	public AddAttributesPortfolioPage(WebDriver driver,ExtentTest test) {
		this.driver = driver;
		this.test=test;
	...
	public WebElement AddattributePageHeader;
	 ...
	 * @throws AWTException 
	 */
	public void addattributedetails(String _Name,String _Description,String _Syskey) throws InterruptedException, AWTException {
...
//
	...
	public String verifyHeaderandIcons_AddlocalizedNamesPopUp(String _AddlocalizedNamesPopUpHeader) throws InterruptedException {
	     status="FAIL";
		    int popup = driver.findElements(By.xpath("//div[14]//span[@class='k-window-title' and text()='"+_AddlocalizedNamesPopUpHeader+"']")).size();
		if(popup==1) {
			status="PASS";
		}
		return status;
...
//		log("Selected the locale of Add localized names as "+_localeForAddlocalizedDescription);
	...
	public String verifyIconLabelsAndHeader(String _Savelabel, String _Cancellabel, String _Deletelabel, String _pageHeaderAddattribute) {
		status="FAIL";
		...
		if(Save.getText().equals(_Savelabel)) {
			...
			if(Cancel.getText().equals(_Cancellabel)) {
				...
				if(Delete.getText().equals(_Deletelabel)) {
					...
					if(AddattributePageHeader.getText().equals(_pageHeaderAddattribute)) {
						log(AddattributePageHeader.getText()+" header is displayed in UI");
