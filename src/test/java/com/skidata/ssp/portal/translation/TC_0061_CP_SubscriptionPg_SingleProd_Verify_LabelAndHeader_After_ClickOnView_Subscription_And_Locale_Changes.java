...
public class TC_0061_CP_SubscriptionPg_SingleProd_Verify_LabelAndHeader_After_ClickOnView_Subscription_And_Locale_Changes extends TestBase {

	public static final Logger log = Logger.getLogger(TC_0061_CP_SubscriptionPg_SingleProd_Verify_LabelAndHeader_After_ClickOnView_Subscription_And_Locale_Changes.class.getName());
	public String status=null;
	DataUtil dataUtil = new DataUtil();

	String testName = "TC_0061_CP_SubscriptionPg_SingleProd_Verify_LabelAndHeader_After_ClickOnView_Subscription_And_Locale_Changes";
	...
	@Test(groups = {"smoke,regression"},timeOut=800000,priority=512,dataProvider = "getDataForTestCaseName", testName = "TC_0061_CP_SubscriptionPg_SingleProd_Verify_LabelAndHeader_After_ClickOnView_Subscription_And_Locale_Changes", description ="TC_0061_CP_SubscriptionPg_SingleProd_Verify_LabelAndHeader_After_ClickOnView_Subscription_And_Locale_Changes")
	public void ssp_TC_0061_CP_SubscriptionPg_SingleProd_Verify_LabelAndHeader_After_ClickOnView_Subscription_And_Locale_Changes(Hashtable<String, String> data)throws Throwable {
...
//				}
			...
			sspReservations.verifyPageTitle(data.get("PageTitle_Subscriptions"));
			Assert.assertEquals(status,data.get("ExpectedValuePASS"),"User not navigated to Reservations page");
			
			sspReservations.clickonBackToSubscriptionLink();
		
			sspReservations.verifyPageTitle(data.get("PageTitle_Subscriptions_Page"));
			Assert.assertEquals(status,data.get("ExpectedValuePASS"),"User not navigated to Reservations page");
			...
			status=sspReservations.verifySingleProduct_HeaderAndLabel_AfterClickOnViewSubscription(data.get("BackToSubscription"), data.get("From"),data.get("ZoomEticket"), data.get("Duration"), data.get("SubscriptionCode"), data.get("IdentificationLabel"), data.get("AddAnotherIdentification_Label"), data.get("TransactionsLabel"),data.get("InvoiceLabel"),data.get("Details"),data.get("BillingAddress"),data.get("ResendMailLink"), data.get("CancelSubscriptionLink"));
			Assert.assertEquals(status,data.get("ExpectedValuePASS"),"Mismatch in the Header and Labels in Profile Subscription page");
